      ...
      <section className="panel">
        ...
        <div className="mt-4 overflow-x-auto rounded-lg border border-border/70">
          <table className="min-w-full text-sm">
            <thead className="bg-[#202329] text-left text-xs uppercase tracking-[0.08em] text-muted">
              <tr>
              ...
              {rows.map((row, index) => (
                <tr className="border-t border-border/60 bg-[#181A1F]" key={row.id}>
                  ...
                  <td className="px-3 py-2">
                    {row.username ? (
                      <Link className="font-medium transition-colors hover:text-[#7C6EFF]" href={`/players/${row.username}`}>
                        {row.name}
        ...
        <div className="mt-4 rounded-lg border border-border/70 bg-[#181A1F] p-3 text-xs text-muted">
          <p>Scoring rules:</p>
